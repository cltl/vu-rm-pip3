#!/bin/bash
#
# Checks if connection to port 2010 (spotlight port for Dutch) 
# is open, and launches spotlight server otherwise.
#-----------------------------------------------------
spotlightdir=$1

if ! lsof -Pi :2010 >/dev/null ; then
  >&2 echo "launching dbpedia spotlight server"
  java -jar -Xmx2000m $spotlightdir/dbpedia-spotlight-0.7.1.jar $spotlightdir/nl http://localhost:2010/rest &>/dev/null &
  
  max=6
  i=0
  while ! lsof -Pi :2010 >/dev/null && [ $i -le $max ]; do
